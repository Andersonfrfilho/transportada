/* Cópia por valor de apps/frontend-client/server.ts (ADR-0075 §7). */
/**
 * Copyright (c) 2026 Ada Technology. MIT License.
 *
 * ⚠️ O estágio de runtime copia só `dist` e este arquivo, então ele não pode importar de `src/`,
 * nem de outra app.
 */
...
/**
 * ADR-0075 §4: câmera e posição abertas para a própria origem (o canhoto e o carimbo da entrega), o
 * valor do painel. HSTS sem `includeSubDomains` e sem `preload`: a zona é da instalação.
 */
...
/** O `sw.js` fica na raiz do `dist`, fora de `/assets/`: sai com `no-cache` e a versão nova chega. */
