...
describe('o smoke só começa quando a app do motorista responde de verdade', () => {
  ...
  /**
   * A 53112 é a origem do `redirect_uri` gravado no build: servir noutra porta quebra o login de
   * verdade, não só a navegação.
   */
  test('a porta do preview vem do ambiente, não de uma segunda flag', () => {
    ...
    expect(CONFIGURATION).toContain('FRONTEND_DRIVER_PORT: String(DRIVER_PORT)')
  })
