 ...
 *
 * ADR-0075 §6, revisão M4: a b18564bc8 estabilizou as lojas padrão do `useDriverTrip` do painel e,
 * sem querer, tirou o único gatilho de repetição que a tela de pendências tinha — o efeito de
 * montagem reexecutando a cada render por causa da loja instável. `scheduleQueueDrainTriggers`
 * (`pendingQueue.service.ts`) é o gatilho explícito que veio no lugar, e este é o teste que prova a
 * montagem: monta o hook de verdade, com o DOM, e conta drenagens — a garantia é que montar dispara
 * uma só, cada gatilho do navegador dispara mais uma, e desmontar corta todas.
 */
...
describe('useDriverTrip — gatilhos da drenagem (revisão M4)', () => {
  beforeEach(() => {
  ...
  it('montar drena uma vez, e online/pageshow drenam mais uma cada — sem emendar', async () => {
     ...
     * montagem reexecutasse a cada render (o defeito que a b18564bc8 corrigiu), a contagem cresceria
     * aqui sem nenhum evento disparado.
     */
    await settle()
    await settle()
