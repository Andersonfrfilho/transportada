...
describe('trip table contract', () => {
  test('sorts only the current page and toggles asc/desc/none per column', async () => {
    ...
    expect(sortTrips(items, { column: 'createdAt', direction: 'desc' })).toEqual([TRIP, SECOND_TRIP])
  })
  ...
  test('counts only filters carrying a defined, non-empty value', async () => {
    ...
    expect(
      countActiveTripFilters({ createdFrom: '2026-07-01', statusEq: 'open', vehicleIdEq: VEHICLE_ID }),
    ).toBe(3)
  })
})
...
describe('trip filter pills contract', () => {
  test('describes only the active filters and clears a single field at a time', async () => {
    const { clearTripFilterField, describeTripFilterPills } = await loadFutureModule<TripFilterPillsModule>(
    ...
    expect(pills.map((pill) => pill.field)).toEqual(['statusEq', 'vehicleIdEq', 'driverIdEq', 'createdRange'])

    expect(clearTripFilterField({ field: 'statusEq', filters })).not.toHaveProperty('statusEq')
    expect(clearTripFilterField({ field: 'vehicleIdEq', filters })).not.toHaveProperty('vehicleIdEq')
...
describe('trip form contract', () => {
  test('requires a vehicle and at least one driver before a trip can be created', async () => {
    ...
    expect(validateTripForm({ driverIds: ['driver-1'], vehicleId: '' })).toEqual(['vehicleRequired'])
...
describe('trip fiscal warning contract', () => {
  test('flags cancelled, denied and rejected documents without disabling the MDF-e action', async () => {
    const { hasTripDocumentFiscalWarning, hasTripFiscalWarning, tripDocumentLabel, tripFiscalStatusKey } =
      await loadFutureModule<TripDocumentModule>('../../src/modules/trip/shared/tripDocument.service')
...
describe('trip feedback contract', () => {
  test('surfaces the first pending mutation error and falls back to requestFailed', async () => {
    const { resolveFirstTripFeedbackKey, resolveTripFeedbackKey } =
      await loadFutureModule<TripFeedbackModule>('../../src/modules/trip/shared/tripFeedback.service')
    ...
    expect(
      resolveFirstTripFeedbackKey([null, new Error('TRIP_DOCUMENT_ALREADY_DELIVERED'), new Error('TRIP_CLOSED')]),
...
describe('trip query status contract', () => {
  test('answers forbidden instead of an eternal loading when the user cannot read trips', async () => {
    ...
    expect(resolveQueryStatus({ canRead: false, isError: false, isPending: true })).toBe('forbidden')
...
describe('trip navigation contract', () => {
  test('navigating to the mdfe manifests screen pushes the path, remembers the workspace and resyncs the shell', async () => {
    const { MDFE_MANIFEST_ROUTE, MDFE_MANIFEST_WORKSPACE, navigateToMdfeManifests } =
      await loadFutureModule<TripNavigationModule>('../../src/modules/trip/shared/tripNavigation.service')
...
function createNavigatorSpy(): Readonly<{ calls: readonly string[]; navigator: WorkspaceNavigatorSpy }> {
  const calls: string[] = []
  return {
    calls,
    navigator: {
