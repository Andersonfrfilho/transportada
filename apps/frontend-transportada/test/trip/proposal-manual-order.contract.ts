...
describe('ordem escolhida à mão na proposta', () => {
  ...
  it('as fichas de carga mudam a ordem de carregamento pelo rascunho', () => {
    ...
    /** Setas também na parada fora do desenho: ela entra no baú como qualquer outra. */
    expect(layers).toContain('{onLoadingMove === undefined ? null : (')
    /** A ordem de carregamento é selo, não linha cinza no meio da ficha. */
    expect(layers).toContain('styles.cargoStopLoadingBadge')
    /** As fichas saem na ordem de carregamento — é a leitura de quem enche o baú. */
    expect(layers).toContain('.sort((first, second) => first.loading - second.loading)')
