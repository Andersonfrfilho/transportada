...
const NUMBER_LABEL_PATTERN = /^N[ºO°]\.?$|^NUMERO$|^N[ÚU]MERO$/iu
...
export function extractCanhotoNumberFromWords(
  ...
  const numberLabelIndex = findLabelIndex(words, NUMBER_LABEL_PATTERN)
  if (numberLabelIndex === -1) return undefined

  const numberToken = words[numberLabelIndex + 1]
  if (numberToken === undefined || !NUMBER_TOKEN_PATTERN.test(numberToken.text.trim())) {
    return undefined
  }
  if (numberToken.confidence < input.minimumConfidence) return undefined
