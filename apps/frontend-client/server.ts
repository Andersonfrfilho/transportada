 ...
 *
 * ⚠️ Cópia por valor do `server.ts` do painel — o estágio de runtime copia só `dist` e este arquivo,
 * então ele não pode importar de `src/`, nem de outra app.
 */
...
const SECURITY_HEADERS: Readonly<Record<string, string>> = {
  ...
  // Aqui as três capacidades são negadas, ao contrário do painel: o portal do cliente não bipa
  // etiqueta (isso é o separador), não rastreia ninguém (quem manda posição é o celular do
  // motorista, no outro app) e não grava áudio. Capacidade aberta que a tela não usa é carona
  // esperando alguém seis meses adiante.
