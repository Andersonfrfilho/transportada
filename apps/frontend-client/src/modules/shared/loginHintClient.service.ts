...
/**
 * Cópia por valor de `loginHintClient.service.ts` do painel. A etapa é anônima e **não tem
...
export async function resolveLoginHint(identifier: string): Promise<string> {
  const response = await fetch(`${getClientEnvironment().apiBaseUrl}/login-hints`, {
