...
const settingsSchema = z
  .object({
    ...
    // Opcionais: cliente anterior à spec 144 não os manda, e a edição preserva o que o perfil tinha.
    nfseEmissionProfileId: z.uuid().nullable().optional(),
    observations: z.string().max(500),
    operationNature: z.string().trim().min(1).max(OPERATION_NATURE_MAX_LENGTH),
    outputDocument: z.enum(CTE_OUTPUT_DOCUMENTS).optional(),
