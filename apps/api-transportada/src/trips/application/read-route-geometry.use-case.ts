...
/**
 * Uma alternativa da rota, pronta para o mapa (spec 093 T1) — a mesma forma que os campos de
...
export type RouteGeometryView = {
   ...
   *
   * ⚠️ **Sempre os da rota principal** — a primeira que o OSRM devolveu (spec 093 D2/spec.md: a
   * alternativa é oferta, nunca troca automática). Quem já lia este campo antes da 093 continua
   * lendo a mesma coisa.
   */
  readonly legs: readonly RouteGeometryLeg[]
  readonly points: readonly { readonly latitude: string; readonly longitude: string }[]
  ...
  /** Por que não há mais barata: ausência de dado, nunca empate (spec 093 D1). */
...
export type ReadRouteGeometryInput = {
  ...
  /**
   * O consumo e o preço do combustível do veículo escolhido (spec 093 D1/T2). Ausente é "não sei
...
export async function readRouteGeometry(input: ReadRouteGeometryInput): Promise<RouteGeometryView> {
  ...
  /**
   * A principal é sempre `options[0]` (spec 093 D2/spec.md): o roteirizador manda no traço padrão,
 ...
 * não é. Cada opção desenha o próprio traço, com o próprio pedágio (spec 093 D3).
 */
