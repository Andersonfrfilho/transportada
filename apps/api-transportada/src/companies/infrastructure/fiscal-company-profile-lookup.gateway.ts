    ...
    async lookupByCnpj({ cnpj }): Promise<CompanyProfileLookupResult | null> {
      try {
        ...
        return {
          ...
          legalNature: normalizeText(result.naturezaJuridica),
          mainActivityCode: normalizeText(result.cnae),
          mainActivityName: normalizeText(result.cnaeDescricao),
          number: normalizeText(result.numero),
          openedAt: normalizeText(result.dataAbertura),
          phone: normalizeText(result.telefone),
          postalCode: onlyDigits(result.cep),
          simplesNacional: result.optanteSimplesNacional === true,
          size: normalizeText(result.porte),
          situation: normalizeText(result.situacao),
