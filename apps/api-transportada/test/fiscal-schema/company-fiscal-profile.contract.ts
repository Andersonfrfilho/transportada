...
describe('tenant fiscal schema', () => {
  test('defines the complete fiscal profile without widening the identity company', () => {
    ...
    expect(checkSqlByName(companyFiscalProfiles)).toEqual({
      ...
      company_fiscal_profiles_cnpj_check: `"company_fiscal_profiles"."cnpj" ~ '^[A-Z0-9]{12}[0-9]{2}$'`,
      ...
      company_fiscal_profiles_mdfe_insurer_tax_id_check: `length("company_fiscal_profiles"."mdfe_insurer_tax_id") = 0 or "company_fiscal_profiles"."mdfe_insurer_tax_id" ~ '^[0-9]{11}$|^[A-Z0-9]{12}[0-9]{2}$'`,
