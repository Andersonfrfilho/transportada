...
describe('fiscal stored object schema', () => {
  test('tracks immutable tenant objects and a recoverable staging lease without storing content', () => {
    ...
    expect(checkSqlByName(storedObjects)).toMatchObject({
      ...
      stored_objects_purpose_check: `"stored_objects"."purpose" in ('import_source', 'nfe_document', 'nfe_event', 'billing_document', 'cte_document', 'mdfe_document', 'nfse_document', 'aggregate_document', 'delivery_proof', 'aggregate_application_attachment')`,
