...
export async function startNfeImportConsumer(params: {
  ...
  return params.provider.consume<NfeProcessingEnvelopeV1>({
    ...
    handler: async ({ payload }) =>
      handleImportEnvelope({ envelope: payload, logger: params.logger }),
    prefetch: params.config.prefetch,
  })
}

async function handleImportEnvelope(params: {
  readonly envelope: NfeProcessingEnvelopeV1
  readonly logger: WorkerLogger
}): Promise<RabbitMqDisposition> {
  if (params.envelope.type !== NFE_PROCESSING_EVENT_TYPE.IMPORT_REQUESTED) {
    safeLogError({
