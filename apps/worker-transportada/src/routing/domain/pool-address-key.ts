 ...
 *
 * ⚠️ **Cópia por valor** de `api-transportada/src/trips/domain/stop-address-key.ts`. As duas apps não
 * importam código uma da outra, e esta é a regra que decide **o que é a mesma parada**: se ela
 * divergir, a parada que o worker propõe e a parada que o aceite cria não casam — e o roteiro
 * aceito ficaria com paradas duplicadas no mesmo portão. `test/routing/pool-address-key.contract.ts`
 * guarda a paridade das duas.
 */
