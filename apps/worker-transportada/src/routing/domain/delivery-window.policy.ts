 ...
 *
 * ⚠️ **Cópia por valor** de
 * `api-transportada/src/delivery-clients/domain/delivery-window.policy.ts`. As duas apps não importam
 * código uma da outra, e esta é a regra que decide **a que horas o cliente recebe** — se ela divergir,
 * o roteiro proposto pelo worker e a conferência feita pela API discordariam sobre o mesmo cliente no
 * mesmo dia. `test/routing/delivery-window-parity.contract.ts` compara os dois arquivos linha a linha.
 */
