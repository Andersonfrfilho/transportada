...
export type ContractorMailInboundWorkerRepository = {
  ...
  /** plan.md § Segurança e tenant: o hash acha a conversa, e `companyId` confere o tenant. */
  findThreadByReplyTokenHash(input: {
    readonly companyId: string
    readonly replyTokenHash: string
  }): Promise<ContractorMailInboundThreadRecord | undefined>
      ...
      const [row] = await database
        ...
        .where(
          and(
            ...
            eq(contractorMailThreads.replyTokenHash, replyTokenHash),
          ),
        )
        .limit(1)
      return row
