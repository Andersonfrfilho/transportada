...
const RELEASE = 'worker-transportada@0.2.0'
...
function createHostileEvent(): Record<string, unknown> {
  return {
    breadcrumbs: [{ category: 'amqp', data: { telefone: PHONE }, message: `consulta de ${EMAIL}` }],
    contexts: { fiscal: { chaveAcesso: ACCESS_KEY, correlationId: CORRELATION_ID } },
    event_id: 'f0e1d2c3b4a5968778695a4b3c2d1e0f',
    exception: {
      values: [{ type: 'CteIssuanceError', value: `emitente ${CNPJ} recusado, contato ${EMAIL}` }],
    },
    extra: { accessKey: ACCESS_KEY, companyId: COMPANY_ID, cpf: CPF },
    level: 'error',
    message: `falha ao emitir para o CPF ${CPF}`,
    request: {
...
describe('contrato do rastreio de erro do worker — no-op sem DSN', () => {
...
describe('contrato do rastreio de erro do worker — opções de privacidade', () => {
  ...
  test('com DSN o rastreio fica ligado e o erro chega ao SDK', () => {
    ...
    const failure = new Error('consumo falhou')
...
describe('contrato do rastreio de erro do worker — redação ponta a ponta', () => {
  ...
  test('identificador opaco e agrupamento do erro sobrevivem', () => {
    ...
    expect(scrubbed).toContain('CteIssuanceError')
