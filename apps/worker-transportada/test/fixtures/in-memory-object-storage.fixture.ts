 ...
 *
 * Dublê em memória do `ObjectStorageProvider` para os testes de integração do worker — o CI deixou
 * de subir o MinIO depois que a MinIO tirou as imagens públicas (2026-09-24). Cópia do dublê da API
 * (`apps/api-transportada/test/fixtures/`): uma app não importa código de outra, nem em teste. As regras são as do provedor
 * real (`@adatechnology/object-storage-provider` 0.3.0; o worker usa a 0.2.0-rc.0, sem `createSignedUpload`), lidas do código dele: gravação só cria,
