...
- [x] **T001** — Contrato: uma declaração só no repositório, e ela mora em `modules/shared/`.
      **Falha contra as oito de hoje**, nomeando cada uma. (P2 · CA1, CA2)
- [x] **T002** — Criar `modules/shared/objectKeys.service.ts` com a guarda em _type predicate_ (D1)
...
- [x] **T003** — Trocar as três posicionais (`trip`, `mdfe-manifest`, `nfse-invoice`), mantendo o
      reexport de cada módulo. Suíte verde após cada uma.
- [x] **T004** — Trocar as cinco de `company-settings`, adaptando a forma de chamada.
...
- [x] **T005** — `make check` verde, smoke verde, e **nenhum teste existente alterado** (CA3) —
