...
## T013 — Gates finais (raiz do worktree, 22/09/2026)

| Gate                                                         | Comando                                                                                                                                                                                                                                                                                                                                                                                                                                     | Resultado                                                                                                                                                        |
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Formatação                                                   | `bun run format:check`                                                                                                                                                                                                                                                                                                                                                                                                                      | ✅ "All matched files use Prettier code style!"                                                                                                                  |
| Tipos                                                        | `bun run typecheck`                                                                                                                                                                                                                                                                                                                                                                                                                         | ✅ exit 0                                                                                                                                                        |
| Lint                                                         | `bun run lint`                                                                                                                                                                                                                                                                                                                                                                                                                              | ✅ exit 0 (`--max-warnings=0` em todas as apps)                                                                                                                  |
| Frontend — testes                                            | `bun run --cwd apps/frontend-transportada test`                                                                                                                                                                                                                                                                                                                                                                                             | ✅ 4806 pass / 0 fail + hooks 44 pass / 0 fail                                                                                                                   |
| Frontend — build                                             | `bun run --cwd apps/frontend-transportada build`                                                                                                                                                                                                                                                                                                                                                                                            | ✅ exit 0 (PWA, 138 entradas no precache)                                                                                                                        |
| API — contratos                                              | `bun --env-file=../../.env.test test --timeout 120000` (em `apps/api-transportada`)                                                                                                                                                                                                                                                                                                                                                         | ⚠️ 6891 pass / 23 skip / **9 fail** — as 9 são as pré-existentes do "toll booth catalog repository (spec 154, T201)", que dependem do Docker; nenhuma falha nova |
| API — integrações novas e vizinhas + completude de migration | `DRIZZLE_TEST_DATABASE_URL=… bun --env-file=../../.env.test test --timeout 120000` com `package-box-unit-estimate`, `package-box-catalog-import`, `migration-completeness`, `package-box-pending-export`, `package-box-replication`, `measurement-history`, `trip-cargo-layout-read`, `trip-cargo-preview-layout`, `trip-detail-query-count`, `trip-document-review`, `mixed-cargo-end-to-end` (Postgres 18 nativo, `127.0.0.1:56998/s163`) | ✅ 72 pass / 0 fail                                                                                                                                              |
| Schema sem diferença                                         | `bun run db:generate`                                                                                                                                                                                                                                                                                                                                                                                                                       | ✅ `{"status":"no_changes"}`                                                                                                                                     |
| Migration + rollback + reaplicação                           | banco novo `s163g`: `db:migrate` → 12 colunas `unit_*`/`estimated_*`; `rollback.sql` (`ON_ERROR_STOP`) → 0; `db:migrate` de novo → 12, e 1 linha no journal                                                                                                                                                                                                                                                                                 | ✅                                                                                                                                                               |
| `make migration-test` (equivalente)                          | `DRIZZLE_TEST_DATABASE_URL=…/s163mt bun --env-file=../../.env.test run db:test` — o alvo do Makefile sobe o Postgres do Docker, quebrado localmente                                                                                                                                                                                                                                                                                         | ✅ 110 pass / 0 fail                                                                                                                                             |

Pendência que não é gate: **CA08 (print da fila)** — ver T012; a revisão por código foi feita, o print
depende de Keycloak/frontend locais de pé.
