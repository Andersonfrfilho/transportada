...
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **0044 adendo**                 | Escalada automática ao provedor pago foi **avaliada e recusada** — _"gasta sem ninguém decidir"_. Há teste guardando: `paid-provider-never-called.contract.ts` | ✅ **ADR-0061** admite um terceiro gatilho — lote decidido, escopo e custo declarados. A recusa da escalada em runtime e o teste que a guarda ficam intactos |
...
- ✅ **D1 fechado pela [ADR-0061](../../docs/adr/0061-o-lote-de-medicao-compra-precisao-uma-vez.md).**
  O lote roda nos 300: a medida de confiança é da base inteira, não só do pedaço quebrado. Ele é um
  **terceiro gatilho** do degrau 2 — decidido por uma pessoa, com escopo e custo na frente —, não uma
  escalada automática, e a recusa que o adendo da **ADR-0044** registrou continua de pé junto com
  `paid-provider-never-called.contract.ts`. ⚠️ A postura das **ADR-0037/0040** (o endereço não sai
  inteiro do navegador; o CEP vem de casa) é o que a 0061 registra por escrito, para não ser herdada
  por silêncio: aqui quem consulta é o **servidor**, uma vez por endereço, e o resultado fica.
