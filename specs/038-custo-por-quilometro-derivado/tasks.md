...
✅ **Liberado para implementar.** Sobra um `[NEEDS CLARIFICATION]` — a janela do CronJob semanal —, e
ele só bloqueia o deploy da Fase F, não o código.
...
- [ ] **T000** 🧠 — Sondar a ANP de verdade e colar em `evidence.md`: (a) a URL corrente da série em
...
- [ ] **T001** — ADR-0033 registrando a escolha: ANP como fonte pública gratuita, arquivo em vez de
      API, CSV em vez de XLSX (dependência), e o ponto que decide o desenho — preço de bomba no
      varejo é sugestão, não verdade. Contexto de comparação: Base dos Dados exige credencial Google
      e atrasa; Apify cobra crédito; Kaggle está paralisado.
...
- [ ] **T002** — Contrato (escrito antes): `test/fleet-schema/` recebe colunas, checks e a unicidade
...
- [ ] **T002b** — Contrato do catálogo: `FUEL_TYPES` com a mesma lista literal nos três lugares
...
- [ ] **T003** — As duas tabelas em `src/database/`, agregadas em `database.schema.ts`, e as duas
...
- [ ] **T004** — Contrato `test/companies/fuel-price-policy.contract.ts`: preço efetivo **por
...
- [ ] **T005** — `src/companies/domain/fuel-price.policy.ts`.
      Verificação: T004 verde.
- [ ] **T006** — Contrato `test/fleet-domain/vehicle-cost.contract.ts`: `deriveCostPerKilometer`
      ...
      Verificação: vermelho.
- [ ] **T007** — `deriveCostPerKilometer` em `src/fleet/domain/vehicle-cost.policy.ts`, ao lado de
...
- [ ] **T008** — Contrato `test/companies/fuel-price.contract.ts`: `GET /company-settings/fuel-prices`
      ...
      Verificação: vermelho.
- [ ] **T009** — Rotas, schema, use cases, porta e repositório de preço por produto.
      Verificação: T008 verde.
- [ ] **T010** — Contrato `test/fleet-http/vehicles.contract.ts`: `POST`/`PUT` de veículo **recusam**
      ...
      Verificação: vermelho.
- [ ] **T011** — `costPerKilometer` fora de `vehicleFieldsSchema`, `fuelType` e
...
- [ ] **T012** 🧠 — Contrato `test/fuel-price-pull/anp-series.contract.ts` usando o **cabeçalho real
      de T000** como fixture: linha válida vira referência; decimal com vírgula é lido certo; a
      tradução do rótulo da ANP para cada um dos cinco produtos do catálogo é assertada nos cinco, com
      o GNV chegando em m³ e não sendo convertido para litro em lugar nenhum;
      linha de produto fora do catálogo é ignorada sem erro; cabeçalho inesperado aborta em vez de
      gravar; contagem de linhas descartadas é reportada.
      Verificação: vermelho.
- [ ] **T013** — `anp-series.client.ts` (HTTP + CSV + Zod na fronteira), a tabela de tradução de
      produto e a política de semana de referência.
      Verificação: T012 verde.
- [ ] **T014** — Contrato `test/fuel-price-pull/run-cycle.contract.ts`: sem lock é no-op com código
      ...
      Verificação: vermelho.
- [ ] **T015** — `run-cycle.ts`, use case, porta, gateway Drizzle, advisory lock e `*.job.ts`;
...
- [ ] **T016** — Contrato: `test/fleet/vehicle-cost.contract.ts` com a **mesma tabela de casos de
      ...
      Verificação: vermelho.
- [ ] **T017** — `deriveCostPerKilometer` em `fleetVehicleCost.service.ts` e a troca da chave em
...
- [ ] **T018** — Preço por combustível em `company-settings`, **uma linha por combustível**: valor efetivo,
...
- [ ] **T019** — `cron-fuel` na tabela de build de `docs/spec/railway.md`, em `INTERNAL_SERVICES` de
...
- [ ] **T020** — `make check` nas quatro apps, `make migration-test`, e a auditoria de go-live: N+1 na
...
- [ ] **T021** — Atualizar `CLAUDE.md`: o trilho novo do cron (e a contagem de jobs, que hoje diz
