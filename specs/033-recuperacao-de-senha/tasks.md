...
- [ ] **T001** 🧠 Migration aditiva: tabela `password_reset_requests` (`id` uuid, `company_id`,
...
- [ ] **T002** Contrato em `test/identity-domain/password-reset.contract.ts`: TTL de 15 min, teto de
...
- [ ] **T003** Implementar `identity/domain/password-reset.policy.ts` e as constantes
...
- [ ] **T004** Contrato de rota em `test/password-reset/http.contract.ts`: `POST /password-resets`
...
- [ ] **T005** Contrato de `POST /password-resets/confirm`: código certo troca a senha e marca
      ...
      certo.
- [ ] **T006** Implementar `request-password-reset.use-case.ts`,
      `confirm-password-reset.use-case.ts`, `drizzle-password-reset.repository.ts`, schemas Zod e as
      duas rotas anônimas; registrar no composition root e no rate limit. Trilha de auditoria com
...
- [ ] **T007** Trilha `password-reset-delivery.v1` (main/retry/dead) na topologia do worker +
      envelope Zod versionado. Contrato do nome das filas junto dos demais.
- [ ] **T008** Consumidor: abre o envelope com AAD
...
- [ ] **T009** Link "Esqueci minha senha" na tela de login e tela em dois passos reusando a forma da
...
- [ ] **T010** `make check` + `make migration-test` + `make worker-integration` verdes; varredura de
